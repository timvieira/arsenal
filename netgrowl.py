...
class GrowlNotificationPacket:
  ...
  def __init__(self, application="growlnotify",
    ...
    self.data += self.application
    self.checksum = md5_constructor() # TIM: md5.new()
    self.checksum.update(self.data)
    if password:
...
def growl(application=None, notification=None, title="Hello, World.",
        description="testing testing 1 2 3 ", priority=0, sticky=False, password=None):
